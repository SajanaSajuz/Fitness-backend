...
const result2 = await trainerModel(reg).save()
if (result2) {
res.status(201).json({ success: true, error: false, message: "trainer added", details: result2 });
}
...
adminRouter.post("/update-trainer", async (req, res) => {
    ...
    try {
      ...
      console.log(data);
      const datass = await trainerModel.updateOne({id: trainer_id , },{$set:data});   //to update
      if(datass){
        return res.status(200).json({success:true,error:false,message:"Trainer Edited",trainer_details: data});
      }
...
adminRouter.get("/trainerdelete/:id", async (req, res) => {
    try {
      ...
      const deleteData= await trainerModel.deleteOne({ _id: req.params.trainerid })
      if(deleteData){
        res.status(201).json({ success: true, error: false, message: "trainer deleted"});
...
adminRouter.get("/productdelete/:id", async (req, res) => {
    try {
      ...
      const deleteData= await productModel.deleteOne({ _id: req.params.productid })
      if(deleteData){
